...
```{r}
tracks(autoplot(Homo.sapiens,which=wh), 
       autoplot(anno.gr[1])
)
...
```{r}
ideo <- Ideogram(genome="hg19")

tracks(ideo,
autoplot(Homo.sapiens,which=wh), 
       autoplot(anno.gr,which=wh)
)
...
vcf <- readVcf("../annotate-and-filter/HCC1143_vs_HCC1143_BL.flagged.muts.vcf","hg19")
vr <- as(vcf,"VRanges")
```
...
strack), from = 10000, to = 20000, cex = 0.8,collapseTranscripts=TRUE)
