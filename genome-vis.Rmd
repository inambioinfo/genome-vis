...
`ggbio` understands about *aesthetics* that we have just introduced for `ggplot2`

```{r}
autoplot(anno.gr[1:10],aes(color=GT.TUMOUR))
```
...
```{r}
autoplot(Homo.sapiens,which=GRanges("chr1",ranges(anno.gr[1:10])))
autoplot(Homo.sapiens,which=GRanges("chr1",ranges(anno.gr[1:10])),stat="reduce")

```


```{r}
tracks(autoplot(Homo.sapiens,which=GRanges("chr1",ranges(anno.gr[1:10]))), 
       autoplot(anno.gr[1:10]))
```
...
```{r}
ideo <- Ideogram(genome="hg19") + xlim(GRanges("chr1",ranges(anno.gr[1:10])))
tracks(ideo,
       autoplot(Homo.sapiens,which=GRanges("chr1",ranges(anno.gr[1:10]))), 
       autoplot(anno.gr[1:10]))
...
library(ggbio)
library(GenomicAlignments)
wh <- GRanges("20",IRanges(1107967,1108280))
reads <- readGAlignments(file="../working-with-vcfs/data/hapmap/NA12878.chr20.bam",param = ScanBamParam(which=wh),use.names = TRUE)
```
